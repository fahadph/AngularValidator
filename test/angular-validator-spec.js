 ...
 describe('angularValidator with form invalid message', function () {
   ...
   beforeEach(inject(function ($rootScope, $compile) {
     ...
     htmlForm = angular.element(
       ...
       '<input ng-model="model.firstName" validate-on="dirty" name="firstName" type="text" required ng-minlength="3" minlength-message="\'Should be 3 characters!\'"  />' +
       '<input ng-model="model.lastName" validate-on="dirty" name="lastName" type="text" required ng-maxlength="5" maxlength-message="\'The last name is too long!\'"/>' +
       '</form>'
     );
   ...
   describe('form invalid message function', function () {
     ...
     it('should show message from customMessage service', function () {
       scope.myForm.firstName.$setViewValue('Jo');
       scope.$digest();

       expect(element.hasClass('has-error')).toBe(true);
       expect(scope.myForm.firstName.$invalid).toBe(true);
       expect(element[0][0].nextSibling.innerHTML === 'Should be 3 characters!').toBe(true);
     });
