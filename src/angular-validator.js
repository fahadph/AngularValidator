    ...
    function($injector, $parse, $compile) {
        return {
            ...
            link: function(scope, element, attrs, fn) {
                ...
                // that are missing it.
                angular.forEach(element.find('input'), function(child_element) {
                    child_element = angular.element(child_element);
                    if (!child_element.attr('name')) {
                        child_element.attr('name', getRandomInt());
                        console.log('WARNING! AngularValidator -> One of your form elements(<input>) is missing a name. We got your back and added a name, but if you want a pretty one you should add it yourself.');
                ...
                angular.forEach(element.find('select'), function(child_element) {
                    child_element = angular.element(child_element);
                    if (!child_element.attr('name')) {
                        child_element.attr('name', getRandomInt());
                        console.log('WARNING! AngularValidator -> One of your form elements(<select>) is missing a name. We got your back and added a name, but if you want a pretty one you should add it yourself.');
                        need_to_recompile = true;
                    }
                });

                angular.forEach(element.find('textarea'), function(child_element) {
                    child_element = angular.element(child_element);
                    if (!child_element.attr('name')) {
                        child_element.attr('name', getRandomInt());
                        console.log('WARNING! AngularValidator -> One of your form elements(<textarea>) is missing a name. We got your back and added a name, but if you want a pretty one you should add it yourself.');
                        need_to_recompile = true;
                    }
                });
                ...
                function updateValidationMessage(element, formInvalidMessage) {
                    ...
                    var defaultMinLengthMessage = function () {
                      var length = element.attributes['ng-minlength'].value ;
                      return "<i class='fa fa-times'></i> Minimum "+length+" characters required";
                    };

                    var defaultMaxLengthMessage = function () {
                      var length = element.attributes['ng-maxlength'].value;
                      return "<i class='fa fa-times'></i> Should be no longer than "+length;
                    };
                    ...
                    if (scopeElementModel.$dirty || scopeForm.submitted) {
                        ...
                        }else if (scopeElementModel.$error.minlength) {
                          if ("minlength-message" in element.attributes) {
                              angular.element(element).after(generateErrorMessage(element.attributes['minlength-message'].value));
                          }
                          // Display the default required message
                          else {

                              // console.log('minlength message',defaultMinLengthMessage());
                              angular.element(element).after(generateErrorMessage(defaultMinLengthMessage));
                          }
                        } else if (scopeElementModel.$error.maxlength) {
                          if ("maxlength-message" in element.attributes) {
                              angular.element(element).after(generateErrorMessage(element.attributes['maxlength-message'].value));
                          }
                          // Display the default required message
                          else {
                              angular.element(element).after(generateErrorMessage(defaultMaxLengthMessage));
                          }
